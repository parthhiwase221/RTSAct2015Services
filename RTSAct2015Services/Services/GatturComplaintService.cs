...
{
    public class GatturComplaintService : IGatturComplaintService
    {
        private readonly IGatturComplaintRepository _repository;
        private readonly IWebHostEnvironment _environment;
        private readonly ILogger<GatturComplaintService> _logger;

        public GatturComplaintService(
            IGatturComplaintRepository repository,
            IWebHostEnvironment environment,
            ILogger<GatturComplaintService> logger)
        {
        ...
        public async Task<ResponseDto<ApplicationResponseDto>> CreateApplicationAsync(GatturComplaintCreateDto createDto)
        {
            try
            {
                _logger.LogInformation("Starting Gattur complaint creation process...");
                ...
                _logger.LogInformation("Gattur complaint created successfully: {ApplicationId}", applicationId);

                return new ResponseDto<ApplicationResponseDto>
                {
                    Success = true,
                    Message = "तक्रार यशस्वीरित्या नोंदविली गेली",
                    Data = new ApplicationResponseDto
                    {
                        Status = "SUCCESS",
                        ApplicationID = applicationId,
                        Message = "तुमची गटार तक्रार यशस्वीरित्या जमा झाली आहे"
                    }
                };
            }
            catch (Exception ex)
        ...
        public async Task<string> SaveDocumentAsync(IFormFile file)
        {
            try
            {
                ...
                var uploadsPath = Path.Combine(webRoot, "uploads", "gattur-complaints");
                ...
                var relativePath = $"/uploads/gattur-complaints/{uniqueFileName}";
        ...
        public async Task<ResponseDto<ApplicationEntity>> GetApplicationByIdAsync(string applicationId)
        {
            ...
            {
                _logger.LogError(ex, "Error getting gattur complaint by ID: {ApplicationId}", applicationId);
                return new ResponseDto<ApplicationEntity>
                {
                    Success = false,
        ...
        public async Task<ResponseDto<IEnumerable<ApplicationEntity>>> GetApplicationsListAsync(
            int pageNumber = 1, int pageSize = 10, string? status = null, string? zoneType = null, string? priority = null, string? searchText = null)
        {
            try
            {
                var applications = await _repository.GetApplicationsListAsync(pageNumber, pageSize, status, zoneType, priority, searchText);
        ...
        public async Task<ResponseDto<bool>> UpdateApplicationAsync(string applicationId, GatturComplaintCreateDto updateDto)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Error updating gattur complaint: {ApplicationId}", applicationId);
                return new ResponseDto<bool>
                {
                    Success = false,
        ...
        public async Task<ResponseDto<bool>> DeleteApplicationAsync(string applicationId, string deletedBy)
        {
            ...
            {
                _logger.LogError(ex, "Error deleting gattur complaint: {ApplicationId}", applicationId);
                return new ResponseDto<bool>
                {
                    Success = false,
