...
{
    public class TreeFellingService : ITreeFellingService
    {
        private readonly ITreeFellingRepository _repository;
        private readonly IWebHostEnvironment _environment;
        private readonly ILogger<TreeFellingService> _logger;

        public TreeFellingService(
            ITreeFellingRepository repository,
            IWebHostEnvironment environment,
            ILogger<TreeFellingService> logger)
        {
        ...
        public async Task<ResponseDto<ApplicationResponseDto>> CreateApplicationAsync(TreeFellingCreateDto createDto)
        {
            try
            {
                ...
                _logger.LogInformation("Tree felling application created: {ApplicationId} with {FileCount} files",
                    applicationId, CountUploadedFiles(createDto));

                return new ResponseDto<ApplicationResponseDto>
                {
                    Success = true,
                    Message = "Tree felling application submitted successfully",
                    Data = new ApplicationResponseDto
                    {
                        Status = "SUCCESS",
                        ApplicationID = applicationId,
                        Message = "Your tree felling application has been submitted successfully"
                    }
                };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error creating tree felling application");
                return new ResponseDto<ApplicationResponseDto>
                {
                    Success = false,
                    Message = "Error submitting application. Please try again.",
                    ErrorCode = "TFL_CREATE_ERROR"
                };
            }
        }
        ...
        /// </summary>
        private async Task ProcessAllFileUploadsAsync(TreeFellingCreateDto dto)
        {
            try
            {
                _logger.LogInformation("Starting tree felling file upload processing...");
                ...
                // **7. Form C&D File**
                if (dto.FormCDFile != null)
                {
                    dto.FormCDFilePath = await SaveDocumentAsync(dto.FormCDFile, "form-cd");
                }

                var uploadedCount = CountUploadedFiles(dto);
                _logger.LogInformation("Tree felling file upload processing completed. Total files processed: {Count}", uploadedCount);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error during tree felling file upload processing");
                throw;
            }
        }
        ...
        public async Task<string> SaveDocumentAsync(IFormFile file, string subfolder)
        {
            try
            {
                ...
                var uploadsPath = Path.Combine(webRoot, "uploads", "tree-felling", subfolder);
                ...
                var relativePath = $"/uploads/tree-felling/{subfolder}/{uniqueFileName}";

                _logger.LogInformation("Tree felling file saved successfully: Original='{OriginalName}' Saved='{SavedPath}' Size={Size} bytes",
            ...
            {
                _logger.LogError(ex, "Error saving tree felling document: FileName='{FileName}' Subfolder='{Subfolder}'",
        ...
        /// </summary>
        private int CountUploadedFiles(TreeFellingCreateDto dto)
        {
            var count = 0;
        ...
        public async Task<ResponseDto<ApplicationEntity>> GetApplicationByIdAsync(string applicationId)
        {
            ...
            {
                _logger.LogError(ex, "Error getting tree felling application by ID: {ApplicationId}", applicationId);
                return new ResponseDto<ApplicationEntity>
                {
                    Success = false,
        ...
        public async Task<ResponseDto<bool>> UpdateApplicationAsync(string applicationId, TreeFellingCreateDto updateDto)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Error updating tree felling application: {ApplicationId}", applicationId);
                return new ResponseDto<bool>
                {
                    Success = false,
        ...
        public async Task<ResponseDto<bool>> DeleteApplicationAsync(string applicationId, string deletedBy)
        {
            ...
            {
                _logger.LogError(ex, "Error deleting tree felling application: {ApplicationId}", applicationId);
                return new ResponseDto<bool>
                {
                    Success = false,
        ...
        public string GenerateApplicationId()
        {
            ...
            return $"TFL{now:yyyyMMdd}-{now:HHmmss}-{Random.Shared.Next(100, 999)}";
        }
    }
}
